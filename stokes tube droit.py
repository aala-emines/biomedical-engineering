...
def withinDistortedArea(n1,n2,rx,ry,ox,oy):
    ...
    for i in range(n2):
        for j in range(n1):
            inVout[count(i,j)] = False#c(i-oy)/c(ry) + c(j-ox)/c(rx) <= 1 or c(i-n2+1)/c(ry) + c(j-ox)/c(rx) <= 1 # true si le point k(i,j) se situe dans une des deux ellipes deformant le cylindre, c-a-d a l'exterieur du cylindre
    return inVout
rx=1
ry =2
ox = int(n1)/2
