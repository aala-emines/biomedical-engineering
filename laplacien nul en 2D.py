...
np.reshape(column,(m,1))
#row, colomn = np.meshgrid(row, column, sparse=True)
X = np.reshape(X,(m,n))# needs to be discussed, the numerotation may be different...
h = plt.contourf(row, column, X)
plt.axis('scaled')
plt.colorbar()
ax.set_title("obtained for (n,m,l,d,c) = {} ".format((150,100,15,10,2)))
plt.show()